...
- **[P3] `@fgv/ts-agent-memory` `IProvenance.derivedFrom` carries the same latent bare-id ambiguity that scope-qualified edges just fixed.**
  ...
  **New follow-up (store scoped-`list()` gap):** `IMemoryRecordSource.list()` now returns `IScopedMemoryRecord[]`, but `IMemoryStore.list()` still returns bare `IMemoryRecord[]`, so the store no longer *structurally* satisfies `IMemoryRecordSource`. `InMemoryCosineIndex.rebuild` has **zero non-test callers today**, so there is no live breakage — but before a consumer can wire `rebuild` against a real store, the store needs a public scoped-listing capability (the internal `IIndexedMemoryRecord.scope` already carries the scope; it just isn't on the public `IMemoryStore` surface). Add a scoped `list()` variant (or an adapter that pairs each `IMemoryStore.list()` record with its resolved `(scope, id)` address) at that point. Purely latent — flagged so the next implementer doesn't rediscover it mid-stream.

  **Still deferred (`derivedFrom` half):** `IProvenance.derivedFrom` remains a bare `MemoryId` — still never dereferenced, still purely latent.

  **Trigger (remaining)**: when a consumer needs to dereference `derivedFrom` back to a specific record.

  **Scope sketch (remaining)**: promote `derivedFrom` to `IEdgeTarget` (or a `{ scope, id }` pair) and fix the ingest stamping site.

  **Not a P2**: `derivedFrom` is never dereferenced today, so there is no functional gap — purely latent.

  **Reference**: the `agent-memory-scoped-edges` stream (scope-qualified `IEdge.target`) deferred both halves; the `agent-memory-scoped-vectors` stream resolved the vector/verdict half.
