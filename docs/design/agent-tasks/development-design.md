...
Proposed default bounds: title 256 characters; description 4,096; reason/outcome/progress summaries 2,048; 32 attention/artifact references per field; 64 scopes; 64 KiB encoded details. Construction may lower these bounds. A3 adds aggregate encoded-byte, identity, count and reserve limits in §8.6; individual field maxima need not fit simultaneously. Required obligations and command receipts consume capacity before acceptance; backpressure rejects new growth without consuming protected completion/cleanup room. References carry identifiers, not automatic dereference permission. Raw binding and detail fields are not implicitly model-visible. **Terminal presentation is not an exception** (decided 2026-09-24, §8.3/T6): a terminal outcome presents its bounded summary and reference identifiers only, and never follows a reference to an executor-owned payload. Reaching that payload is a separately-authorized host action. This had been carried as an open question through T1–T5 — it reached none of them, since the broker never dereferences a reference, the default projection removes artifact references, and an update's presentation is the bounded envelope frozen with its revision — and is settled here rather than in T6, so the slice that first holds executor-owned payloads builds against a stated contract.
...
**Source-history spelling (confirmed by T6, 2026-09-24).** The history contract is spelled `'observed-state' | 'source-replay'` everywhere, as §1 and §8.6 spell it; this section's sketch previously said `'latest-snapshot' | 'replayable-updates'`. §8.6's names were kept because they name the *guarantee* a registration relies on (and T1's `SourceHistoryDeclaration` and the `admitted-source-replay` claim are already built on them), where the older names described a mechanism.

**As implemented (T6).** `ITaskSource` ships as sketched with these recorded differences: `capabilities()` is not part of it yet (its pause/cancel declarations are consumed by the stop slice, T9, which adds it); an optional `lookupCommand` resolves uncertain receipts; a reconcile page states its `coverage` (`all-bindings` | `active-only`); `SourceCommandResult.rejected` carries a closed reason and gains a `key-expired` state; and `RecoveryResult.unrecoverable` carries the source's own failed/cancelled projection so the broker never invents the failure it applies. The package's `CAPABILITIES.md` is authoritative for the shipped surface.
