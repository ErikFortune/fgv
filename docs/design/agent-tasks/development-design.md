...
**Status:** durability boundary and bounded stop presets approved 2026-09-21; capacity amendment A3 pending orchestrator/user approval; awaiting explicit implementation authorization. No production implementation, test execution, or memory measurement is claimed.
...
3. **Residency and finite capacity — A3 recommended, not yet approved:** use minimal archived entries, bounded caches and staged rebuild (§7), plus persisted capacity claims that reserve room to finish accepted work (§8.6). Keep physical deletion, inventory compaction and acknowledgement/dedup-history compaction deferred only with the finite-lifetime operational limitation below. This adds admission/accounting work and constrains source-replay admission; it is not merely a documentation correction.
...
| Encoded records | Task, consumer and inventory each 8 MiB; source record 1 MiB. Include unused claims for that record's future growth. |
...
A1 and A2 in §1 remain approved. **A3 requires orchestrator/user approval** of the finite-horizon capacity policy, protected completion/accounting work, proposed initial limits and source-replay qualification; alternatively, approve a separate compaction design before implementation/adoption requiring indefinite retention. These choices authorize neither an implicit consumer rollout nor other deferred features. After design approval and explicit implementation authorization, begin with the isolated upstream FileTree capability contract and qualification fixtures; only after that gate passes build the durable task repository. Approval of the guarantee is not evidence that implementation, capacity qualification or measurements have passed.
