...
### `ai-assist-structured-output` ✅ (shipped 2026-08-22 — breaking for constructors, additive for readers)

**Status:** ✅ shipped to `release`. Gates green — build / lint / test at **100%** in `@fgv/ts-extras` (2733 tests) and `@fgv/ts-json-base`, repo-wide `rush rebuild` at exit 0 with zero warnings, change files for all three published packages. **One gate is deliberately left open**: live per-provider testbed verification, which needs API keys this environment does not have — the four wire shapes are pinned against the documented forms, which is evidence the code does what it intends and *not* evidence the providers accept it.
