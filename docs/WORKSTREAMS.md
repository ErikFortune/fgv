...
**One slip worth recording, because it is the second instance in two days.** Inserting the new capability resolver above `resolveEmbeddingCapability` consumed that function's TSDoc block, producing an `ae-missing-release-tag` warning — the identical mistake the `result.ts` split made 24 hours earlier when its cut took `Success`'s doc block. Both were caught by the build; the pattern is that a text insertion anchored on an `export function` line lands *inside* the preceding symbol's documentation.
