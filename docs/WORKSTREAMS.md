...
**Status:** 🟢 implemented — PR open onto `release`; branch `agent-memory-ingest-dedup-scope` from `release` @ `b392e1534`; brief at `.ai/tasks/active/agent-memory-ingest-dedup-scope/brief.md`, result at `.ai/tasks/active/agent-memory-ingest-dedup-scope/result.md`. All five deliverables landed; 721 tests green at 100% coverage. Runs in parallel with `safer-fetch-s3`; no code overlap, but both edit `.ai/instructions/LIBRARY_CAPABILITIES.md` and this file — **own section only**.
