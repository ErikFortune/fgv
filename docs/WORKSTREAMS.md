...
**Status:** ✅ shipped via **#663**, stacked on #661 (same pattern as #662). Gates green — build / lint / test at **100%** in `@fgv/ts-extras` (2787 tests) and `@fgv/ts-prompt-assist` (280), repo-wide `rush rebuild` at exit 0 with **zero warnings**, change files for both verified against `origin/release`, `@fgv/testbed` (the one downstream consumer) green. **No coverage directives added.**
...
**An invariant check written expecting to be unreachable turned out to be load-bearing.** `renderWithSegments` compares its accumulated text against `render()` before returning. A **set-delimiter tag** reaches it: it changes the delimiters for every following tag, while each interpolation here is rendered from its own template slice, which the writer reads with the *default* delimiters — so `A{{=<% %>=}}<%v%>B` renders `AXB` but would have been mapped as `A<%v%>B`. The probe cost one `node -e` line and converted a would-be `c8 ignore` into a documented limitation with a named cause. It also proved the surface's own TSDoc wrong ("set-delimiter tags are simply skipped"), along with the test named after it, which had only ever exercised a comment.
