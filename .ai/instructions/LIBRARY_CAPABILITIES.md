...
- **Validating a JSON-shaped LLM response against a typed `Converter<T>` + validator chain (with no cast at the call site)?** → `PromptLibrary.resolveAndValidateOutput<T>(req, rawOutput)` from `@fgv/ts-prompt-assist`. Build a `PromptRegistry<TResponse>` where `TResponse` is the consumer's response union (each member carries a `kind: '<discriminator>'` field). Register Converters via `registry.converters.register(id, kind, converter)` and validators via `registry.outputValidations.register(id, { appliesTo, validate })`. The pipeline strips Markdown fences (via `AiAssist.extractJsonText`), `JSON.parse`s, dispatches to the Converter recorded under `(id, kind)`, then chains the validators (each narrowed by `value.kind ∈ appliesTo`). Loader-side checks reject incompatible descriptors at `describe()` / `resolve()` time (belt); the runtime path re-checks `value.kind` per validator (suspenders).
- **Enforcing safety policies (length cap, suspicious-pattern screen, source-aware skipping, anti-jailbreak preface) on a resolved prompt?** → `IPromptLibraryCreateParams.safetyPolicy` from `@fgv/ts-prompt-assist`. Carries `defaultMaxLength`, `suspiciousPatterns` (RegExp screen with `lastIndex` reset between slots so `g` / `y` flags don't leak), `screenedSources` (slot-source allowlist), `onSuspicious` (`'warn'` | `'reject'`), and `antiJailbreakPreface` (consumer-supplied post-render framing — library ships no default content). All findings surface in `trace.safeguardFindings`.
