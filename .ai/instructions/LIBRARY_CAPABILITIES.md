...
- **Need a private-key storage backend so `KeyStore.addKeyPair` works?** → **Node:** `EncryptedFilePrivateKeyStorage.create({ directory, encryptionKey, cryptoProvider })` from `@fgv/ts-extras/crypto-utils` (one AES-256-GCM-encrypted JWK file per key via FileTree; you supply the 32-byte `encryptionKey`; `supportsNonExtractable: false`). **Browser:** `IdbPrivateKeyStorage.create({ databaseName?, storeName? })` from `@fgv/ts-web-extras/crypto-utils` (IndexedDB; stores `CryptoKey` directly; `supportsNonExtractable: true`). Pass the instance as `KeyStore.create({ ..., privateKeyStorage })`. Don't roll your own `IPrivateKeyStorage`.
