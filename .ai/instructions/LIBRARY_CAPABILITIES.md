...
**Self-diagnosing setup.** `MessagesProvider` self-diagnoses missing Tailwind content-path setup via an in-panel warning message on mount — a one-shot probe measures whether `h-3.5` resolves to a sized element, and if it does not, appends a `'warning'` message with an `action.href` linking to the README setup section. Idempotent under React.StrictMode double-render and HMR; always-on (not dev-only). `IMessageAction` accepts either `href` (external link) or `onAction` (callback).
