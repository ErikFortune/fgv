...
- **Need stable canonical JSON (RFC 8785)?** → `HashingNormalizer.canonicalize()` (or `Crc32Normalizer.canonicalize()`) from `@fgv/ts-utils` hash packlet.
...
- **Need X25519 key agreement?** → `generateKeyPair('x25519', ...)` from `ICryptoProvider` (`@fgv/ts-extras/crypto-utils`), then `crypto.subtle.deriveBits({ name: 'X25519', public: peerPublicKey }, privateKey, 256)` for the shared secret.
