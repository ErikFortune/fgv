...
- **Validating a JSON-shaped LLM response against a typed `Converter<T>` + validator chain?** → `PromptLibrary.resolveAndValidateOutput<T>(req, rawOutput)` from `@fgv/ts-prompt-assist`. Build a `PromptRegistry<TResponse>` where `TResponse` is the consumer's response union (each member carries a `kind: '<discriminator>'` field). Register Converters via `registry.converters.register(id, kind, converter)` and validators via `registry.outputValidations.register(id, { appliesTo, validate })`. The pipeline strips Markdown fences (via `AiAssist.extractJsonText`), `JSON.parse`s, dispatches to the Converter recorded under `(id, kind)` (no cast between the Converter type and validator inputs), then chains the validators (each narrowed by `value.kind ∈ appliesTo`). Loader-side checks reject incompatible descriptors at `describe()` / `resolve()` time (belt); the runtime path re-checks `value.kind` per validator (suspenders). The public generic `T extends TResponse` on `resolveAndValidateOutput<T>` is a caller assertion — the belt+suspenders constrain it structurally but do not bind `T` to the descriptor's converter id, so requesting `<IClassifierResponse>` for a descriptor whose converter produces `ICitedResponse` is statically allowed and not runtime-checked. Two patterns avoid the wrong-member trap: bind `T` when you know the converter id's produced shape, OR omit the type argument so `T` defaults to `TResponse` and discriminate on `value.kind` after the resolve (binding `T` to a single union member and then trying to `if (value.kind === 'other')` is a no-op — TypeScript already narrowed).
- **Enforcing safety policies (length cap, suspicious-pattern screen, source-aware skipping, anti-jailbreak preface) on a resolved prompt?** → `IPromptLibraryCreateParams.safetyPolicy` from `@fgv/ts-prompt-assist`. Carries `defaultMaxLength`, `suspiciousPatterns` (RegExp screen with `lastIndex` reset between slots so `g` / `y` flags don't leak), `screenedSources` (slot-source allowlist), `onSuspicious` (`'warn'` | `'reject'`), and `antiJailbreakPreface` (consumer-supplied post-render framing — library ships no default content). Warn / info findings (`'suspicious-pattern'` under `'warn'`, `'screening-skipped'`, `'enforced-override-ignored'`) surface in `trace.safeguardFindings`; reject paths (length-cap violations, `'suspicious-pattern'` under `'reject'`) fail the resolve with the rejection in the error message rather than on a trace.
- **Mustache rendering for LLM prompts (verbatim, no HTML escaping)?** → `MustacheTemplate.create({ escape: 'none' })` from `@fgv/ts-extras` (mustache packlet). Uses a per-instance `Mustache.Writer` — no mutation of the global `Mustache.escape`, so concurrent renderers with different escape strategies coexist. Default `'html'` preserves the existing back-compat behaviour; pass a `(value) => string` callback for a custom strategy. `@fgv/ts-prompt-assist` consumes the `'none'` strategy internally.
