# ts-prompt-assist v0.1 — consumer pressure-test findings (round 1)
...
**Round 1 status (orchestrator triage):**
- Integration code lives on PR #373 — awaiting browser smoke-test before merge.
- Adjudication outcome: a focused ergonomics PR cycle (ts-utils → ts-res → ts-prompt-assist) addresses **F1, F3, F9, F12, F13, F14** before consumer port; **F6 dropped** (the sync-fixture proposal would break fixture-→-FileTree migration symmetry — covered instead by F9's React-wiring doc); **F7, F8 deferred to v0.2** (wait for real consumer evidence); **F5, F10, F11 retracted** by the agent on review.
- Round 2 will land at `pressure-test-findings-round-2.md` after the ergonomics PRs ship and the pressure-test agent re-runs against the updated surface.
