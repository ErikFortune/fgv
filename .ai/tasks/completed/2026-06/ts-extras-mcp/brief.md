...
- **Headline adapter:** `adaptMcpTools(session, { logger? }): Promise<Result<{ tools: ReadonlyArray<IAiClientTool>; skipped: ReadonlyArray<{ name: string; reason: string; schema: JsonValue }> }>>` (or equivalent satisfying Constraint 1). `schema` is `JsonValue` (not `JsonObject`) — the raw failing `inputSchema` is carried verbatim and can be `null` / non-object when a server advertises an invalid schema. Each adapted tool's `parametersSchema = JsonSchema.fromJson(inputSchema)` and `execute: (args) => callMcpTool(session, name, args)` mapping `isError`→fail. Use `mapResults` / per-item helpers per repo Result conventions.
