...
| **B-1a — Foundation (no ts-res integration)** | Agent | **Landed via [#362](https://github.com/ErikFortune/fgv/pull/362) 2026-05-16.** Types + Converters for string-unions / branded scalars / discriminated unions + descriptor / bindings / qualifier-axis Converters + unified `IPromptRegistry<TResponse>` (per design §17.2) + chain walker + binding merger (cross-scope merge with `enforced` lock + `winningScope` trace) + Mustache render via B-0b `escape: 'none'` + InMemoryFileTree test fixture (`PromptStoreFixture.build(seed)`) + happy-path `PromptLibrary.resolve` returning full `IPromptResolveTrace`. ts-res integration deferred to B-1b (the B-1 agent's in-record candidate selector is a self-contained matcher; it does not drive the long-lived `ResourceManagerBuilder` from design §15.5 Option C). 71 tests, 100% coverage. Code-reviewer P1 findings fixed before merge. | Opus | Originally scoped as a single "B-1" run; agent surfaced the ts-res integration scope honestly and deferred to a follow-up rather than shipping silent placeholders (Guardrail #4 respected). Orchestrator formalized the B-1a/B-1b split post-merge. |
| **B-1b — ts-res integration + B-1 review deferrals** | Agent | **Authoritative detail in `.ai/tasks/active/ts-prompt-assist/brief-phase-b-1b.md`** (the B-1 agent pre-drafted the per-deferral catalog; orchestrator endorses). Summary: **(1) ts-res integration per design §15.5 Option C / §17.1** — long-lived `ResourceManagerBuilder` inside `PromptLibrary`, parameterized by the new `qualifiers` create-param (`IReadOnlyQualifierCollector \| ReadonlyArray<IQualifierDecl>`); lazy `addLooseCandidate` on first-resolve cache miss keyed by `Crc32Normalizer.computeHash(record.candidates)`; `ResourceResolver` over the builder drives candidate selection (replacing the B-1a in-record matcher; the standalone matcher is REMOVED, not retained as a fallback). Trace surfaces ts-res match disposition (`'match' \| 'matchAsDefault'`) and per-condition `IConditionMatchResult[]` (both were placeholders in B-1a). New create-params: `qualifiers`, `qualifierTypes?`, `cacheListener?`, `logger?`, `resourceBindingDepthLimit?` (the last is wiring-only; resource binding resolve itself stays B-2). **(2) Sixteen Copilot deferrals from PR #362** — see `brief-phase-b-1b.md` §"Part 2" for each with file:line + recommended fix path. Highlights: `SlotName` tightens to Mustache-identifier regex; `Convert.promptId` rejects `::` (eliminates `MustacheTemplateCache` key-collision risk); `ConverterRegistry.get` gains kind-verified overload; `promptFileConverter` stops double-traversing; `_slotBindingHolder` mutable singleton replaced; `PromptStoreFixture` mapResults chained for short-circuit; `chainWalker` parallelizes `getBindings`; `describe` cache validates structural equality + exposes `invalidateDescriptor`; reserved Win device set widens to `COM0..9` / `LPT0..9`. Several of the 16 close as documentation-only (design rationale rather than deferred work). **(3) Refactor imperative `isFailure()` cascade in `resolve()` to Result chaining** (Guardrail #10, code-reviewer P2 from B-1a). | Opus (ts-res has subtle behavior; B-1a agent flagged the deferral specifically because the integration is non-trivial) | B-1b out-of-scope: resource binding resolve (B-2); FileTree disk path + filename-id consistency + FsTree smoke test (B-3); output validation pipeline (B-4); input safeguards beyond `'enforced-override-ignored'` (B-4); LIBRARY_CAPABILITIES + README (B-5). The B-1a in-record matcher is REPLACED by ts-res integration; it is not retained as a fallback (no dual-path code). |
| **B-2 — Resource bindings** | Agent | Recursive resolver; cycle detection via **RFC 8785 canonical-JSON keys on `(scopeChain, promptId)` pairs** — NOT CRC32 (per B-1b PR #364 R2 finding: CRC32 isn't collision-resistant; canonical-JSON is what the rest of the library now uses for cross-scope equality + materialization keys; consistent + collision-free). NEVER `JSON.stringify`-compare. Depth cap (default 5, wired through `IPromptLibraryCreateParams.resourceBindingDepthLimit` from B-1b). OQ-2 strict-replace substitutions semantics with `'replace' \| 'inherit'` trace. Full `innerTrace: IPromptResolveTrace` recursive surfacing in `IResourceBindingTraceEntry`. Sub-phase ends with resource bindings working against the InMemoryFileTree fixture (no FileTree disk path yet). Operates against the B-1b ts-res-integrated foundation — inner resolves flow through the same long-lived `ResourceManagerBuilder` materialization path (no parallel resource manager). | Opus (recursive resolution + cycle detection + ts-res integration tendrils) | Code-reviewer pass mandatory. |
...
| **B-4 — Output validation pipeline + Input safeguards** | Agent | **Output (per design §8 + §17.2):** fence strip via `extractJsonText`; `JSON.parse`; resolve Converter by `output.converterId` (returns typed `T extends TResponse`); chain through `outputValidations[]` (each validator typed against `TResponse`; `appliesTo` narrows; runtime-fail safety net per §17.2.4); loader-side reject when validator `appliesTo` doesn't match Converter's declared response kind (belt). **Safeguards (per design §9):** per-slot length cap, regex screen (with `pattern.lastIndex = 0` reset for stateful flags), source-aware skipping, anti-jailbreak preface seam, full `safeguardFindings[]` surfacing including the `'enforced-override-ignored'` kind. **Gate (carried from B-2 PR #366 review):** `IBindingMergeResult.mergedBindings: ReadonlyMap<SlotName, SlotBinding>` is currently built by `bindingMerger` but read by nothing. B-4 MUST either (a) consume `mergedBindings` (e.g. to feed `IOutputValidationContext.substitutions` with the raw `SlotBinding` shape rather than the entry-typed view) and document the consumer, OR (b) remove the field from `IBindingMergeResult` and stop populating it. The current shape is dead-weight that survived B-1 → B-2 only because no one had a use for it; do not perpetuate it into v0.1 without a justified consumer. | Sonnet | Combined since both are small post-resolve stages. Code-reviewer pass mandatory. |
