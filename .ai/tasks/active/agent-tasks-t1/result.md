...
| `UpdateCategory` | 7 | count only | the count is the arithmetic input to closeout | **prediction.** No update is constructed in T1; the seven names come from design §8.3 <br>**T2:** now exercised — `attention` (when required) and non-`progress` categories drive priority, and categories are rendered. T2 enforces one update per `(task, revision, category)`, which bounds a receipt entry to seven update IDs; **T7 must fit baseline obligations to that or revise it.** No member revised. <br>**T3:** update identity is now `taskUpdateId(task, revision, category)` = `<taskId>:<revision>:<ordinal>`, checked on every stored update; the category ordinal is part of the identity, so **reordering `allUpdateCategories` is now a storage-format change.** The update-id bound grew by `maxUpdateIdSuffixLength` (19) so a maximum-length task id still has one. <br>**T4:** the category ordinal also orders the owed-update index key (`task, revision, ordinal`) — in memory only, derived at open, so reordering still changes storage identity (T3) and nothing more. No member revised. <br>**T5:** first producer of updates from real mutations — `lifecycle`, `result`, `attention`, `progress` (also title/description), `assignment` (reassign), `relationship` (reparent **and** scope changes). `required` is false for `progress` and `observation` only. An update owed to no one is not retained, so with no subscriptions (T7) none is written. No member revised. |
| `ParentStopPolicy` | 3 | **3/3** | each converts; unknown fails | whether v1's immutability holds (T9) <br>**T5:** chosen at tracked/list creation (default `none`), `none` for external registrations, and carried unchanged by every broker mutation. Its choice is still T9's. No member revised. |
| `TrackedTaskCommandName` | 11 | names only | each satisfies the command-name syntax; none is an external `setStatus` | **the parameter schemas do not exist.** Deliberately: they belong to the slice implementing the transitions <br>**T5: parameter shapes decided** (`TrackedCommand`): `start`/`resume` `{}`, `wait` `{ reason: IWaitingReason }`, `pause` `{ reason }`, `succeed` `{ outcome }`, `fail`/`cancel` `{ reason, outcome? }`, `set-title` `{ title }`, `set-description` `{ description? }` and `set-progress` `{ progress? }` (omitted = clear), `set-attention` `{ attention }`. Validated by strict converters; **not** registered as kind command handles — the tool/wire schema is I1's. No name revised. |
| `TaskListCompletion` | 2 | **2/2** | both convert; unknown fails | whether `all-children-succeeded` needs sub-modes (T5) <br>**T5: settled, no sub-modes.** `all-children-succeeded` = at least one child and every child in the complete authoritative set resolved and succeeded, pumped by the host; `manual` = explicit `completeList`, which also requires every child succeeded (and is the only way an empty list completes). No member revised. |
...
**T5 addendum (2026-09-23).** T5 revised no T1 member and **no persisted shape**: catalog
requests and receipts are still stored as validated JSON in `IStoredCatalogOperation`, now with
typed converters per operation; no record T1–T4 wrote needs migration. One index shape widened —
list-completion candidates and a per-parent succeeded-child count in `TaskIndex`, derived and
rebuilt by `rebuildIndexes()`/open, never persisted — and `ITaskRepository` gained `childStates`
and `listCompletionCandidates`. New closed sets: `TaskAction` (14), `TaskAccessRole` (4),
`TaskMutationDisposition` (2); `TrackedCommand` gives T1's command names their parameters. Details
in `.ai/tasks/active/agent-tasks-t5/result.md`.
