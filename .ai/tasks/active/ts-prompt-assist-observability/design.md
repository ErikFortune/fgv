...
| **OQ-6** | **Headline (Q1):** ship `RetainingRingBuffer<T>` in `@fgv/ts-utils` (hybrid C-minimal + D), or stay pure-D with a hand-rolled ring inside `ts-prompt-assist`? | **LOCKED (orchestrator, 2026-06-04): hybrid C-minimal + D.** Phase C ships the generic buffer + the schema-aware store. `RetainingLogger` is **not** refactored to compose the buffer in Phase C (keeps Phase C additive) — that refactor is a **committed fast-follow** scheduled to land soon after Phase C so the two ring implementations don't diverge. |
