...
| controllable source really applies | reverting `SimulatedExecutor._apply` to a no-op turns **16** tests red |
...
its tests red: replay hint applied directly (1), cursor saved past a capacity block, second send
after marker, non-idempotent resend (3), no settlement reservation (13), accepted recorded as
applied, executor stops applying (16), same-revision conflict accepted, envelope overdraw (1), no
authority recheck at dispatch (1), stale precondition (1), active-only counted complete (1), no feed
order check (1), no subject check at marker (1), cursor saved before observations (5), no epoch
fence at resend (1), no subject fence at resend (1), resend after another caller settled (1).
...
`verify-tarball-exports` — results recorded in the PR.
