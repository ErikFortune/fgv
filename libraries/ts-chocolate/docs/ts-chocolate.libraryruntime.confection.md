<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [@fgv/ts-chocolate](./ts-chocolate.md) &gt; [LibraryRuntime](./ts-chocolate.libraryruntime.md) &gt; [Confection](./ts-chocolate.libraryruntime.confection.md)

## LibraryRuntime.Confection class

Static factory for creating runtime confections. This class cannot be instantiated - use create() to get the appropriate concrete type.

**Signature:**

```typescript
export declare abstract class Confection 
```

## Example


```typescript
const result = Confection.create(context, id, confection);
if (result.isSuccess()) {
  const confection = result.value;
  if (confection.isMoldedBonBon()) {
    console.log(confection.shellChocolate);
  }
}
```

## Methods

<table><thead><tr><th>

Method


</th><th>

Modifiers


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

[create(context, id, confection)](./ts-chocolate.libraryruntime.confection.create.md)


</td><td>

`static`


</td><td>

Factory method that auto-detects confection type and returns appropriate concrete class.


</td></tr>
</tbody></table>

