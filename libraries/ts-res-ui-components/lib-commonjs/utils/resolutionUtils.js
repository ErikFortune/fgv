  ...
  return ts_res_1.Runtime.ValidatingSimpleContextQualifierProvider.create({
    ...
    .onSuccess((contextProvider) => {
      ...
      if (errors.hasMessages) {
        return (0, ts_utils_1.fail)(`Errors setting context values: ${errors.toString()}`);
      }
      // Create resolver
      ...
      if (options.enableCaching) {
        const metricsListener = new ts_res_1.Runtime.ResourceResolverCacheMetricsListener(
          () => new ts_res_1.Runtime.AggregateCacheMetrics()
        );
        resolverParams.listener = metricsListener;
      }
      return ts_res_1.Runtime.ResourceResolver.create(resolverParams)
        .withErrorFormat((e) => `Failed to create resolver: ${e}`)
        .onSuccess((resolver) => {
          debugLog(enableDebug, 'Resolver created successfully');
          return (0, ts_utils_1.succeed)(resolver);
        });
    });
}
/**
