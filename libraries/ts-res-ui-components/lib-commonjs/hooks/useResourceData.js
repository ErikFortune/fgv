...
function useResourceData(params) {
  ...
  const processDirectoryWithConfig = (0, react_1.useCallback)(
  ...
  const processFiles = (0, react_1.useCallback)(
    async (files) => {
      setState((prev) => ({ ...prev, isProcessing: true, error: null }));
      try {
  ...
  const processBundleFile = (0, react_1.useCallback)(
    async (bundle) => {
      ...
      try {
        ...
        const { systemConfiguration, compiledCollection, metadata } =
          ts_res_1.Bundle.BundleUtils.extractBundleComponents(bundle).orThrow(
        ...
        const bundleResourceManager = ts_res_1.Bundle.BundleLoader.createManagerFromBundle({
          bundle: bundle,
        ...
        const editableResourceManager =
          ts_res_1.Resources.ResourceManagerBuilder.createFromCompiledResourceCollection(
            compiledCollection,
            systemConfiguration
        ...
        const system = {
          ...
          importManager: ts_res_1.Import.ImportManager.create({
            resources: ts_res_1.Resources.ResourceManagerBuilder.create({
              qualifiers: systemConfiguration.qualifiers,
              resourceTypes: systemConfiguration.resourceTypes
            }).orThrow()
          }).orThrow(),
          contextQualifierProvider: ts_res_1.Runtime.Context.ValidatingSimpleContextQualifierProvider.create({
            qualifiers: systemConfiguration.qualifiers
          }).orThrow()
        };
        // Extract configuration for UI display using the new getConfigurationJson method
        const allQualifierTypes = (0, ts_utils_1.mapResults)(
          Array.from(system.qualifierTypes.values()).map((qt) =>
            qt
              .getConfigurationJson()
              .onSuccess((jsonConfig) =>
                ts_res_1.QualifierTypes.Config.Convert.anyQualifierTypeConfig.convert(jsonConfig)
              )
          )
  ...
  const resolveResource = (0, react_1.useCallback)(
    (resourceId, context) => {
      if (!state.processedResources?.system?.resourceManager) {
        return (0, ts_utils_1.fail)('No resources loaded');
      }
      try {
        ...
        return state.processedResources.system.resourceManager
          ...
          .onFailure(() => (0, ts_utils_1.fail)(`Resource not found: ${resourceId}`))
          .onSuccess(() =>
            (0, resolutionUtils_1.createResolverWithContext)(state.processedResources, contextValues, {
              enableDebugLogging: false
            }).withErrorFormat((msg) => `Failed to create resolver: ${msg}`)
          )
          .onSuccess((resolver) =>
            (0, resolutionUtils_1.resolveResourceDetailed)(resolver, resourceId, state.processedResources, {
              enableDebugLogging: false
          ...
          .onSuccess((resolutionResult) => {
            ...
            return (0, ts_utils_1.succeed)(detailedJson);
          });
      } catch (error) {
  ...
  const applyConfiguration = (0, react_1.useCallback)((config) => {
    setState((prev) => ({ ...prev, activeConfiguration: config }));
  }, []);
  const updateProcessedResources = (0, react_1.useCallback)((processedResources) => {
    setState((prev) => {
