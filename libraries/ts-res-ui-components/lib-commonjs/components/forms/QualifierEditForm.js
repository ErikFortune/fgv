...
const QualifierEditForm = ({ qualifier, qualifierTypes, onSave, onCancel, existingNames = [] }) => {
  ...
  const allowsContextList = (() => {
    ...
    const result = ts_utils_1.Converters.boolean.convert(
      selectedQualifierType.configuration.allowContextList
    );
    ...
    switch (selectedQualifierType.systemType) {
      ...
      case 'literal':
        ...
        const enumValues = (() => {
          ...
          const result = ts_utils_1.Converters.arrayOf(ts_utils_1.Converters.string).convert(
