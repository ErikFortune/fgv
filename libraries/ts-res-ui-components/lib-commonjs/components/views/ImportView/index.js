...
const ImportView = ({
  ...
  const dirInputRef = (0, react_1.useRef)(null);
  ...
  const processZipFile = (0, react_1.useCallback)(
    async (file) => {
      ...
      try {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          try {
            ...
            } else if (ts_res_1.Bundle.BundleUtils.isBundleFileName(file.name)) {
  ...
  const handleDirectorySelect = (0, react_1.useCallback)(
    async (event) => {
      const files = event.target.files;
      if (!files || files.length === 0) return;
      setIsLoading(true);
  ...
  const handleModernDirectoryPick = (0, react_1.useCallback)(async () => {
  ...
  const handleModernFilePick = (0, react_1.useCallback)(async () => {
