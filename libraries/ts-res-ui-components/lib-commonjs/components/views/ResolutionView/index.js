...
const UnifiedChangeControls_1 = require('./UnifiedChangeControls');
...
const ResolutionResults_1 = require('../../common/ResolutionResults');
const NewResourceModal_1 = require('./NewResourceModal');
...
const ResolutionView = ({
  ...
  const [showNewResourceModal, setShowNewResourceModal] = (0, react_1.useState)(false);
  // Local toggles for editing/creation features (controllable via options dialog)
  const [allowResourceCreationInternal, setAllowResourceCreationInternal] = (0, react_1.useState)(
    !!allowResourceCreation
  );
  const [showPendingResourcesInListInternal, setShowPendingResourcesInListInternal] = (0, react_1.useState)(
    ...
    allResourceIds.forEach((resourceId) => {
      ...
      if (hasResolutionResult && resolutionState?.resolutionResult?.success) {
        ...
        if (result.bestCandidate) {
          ...
          const matchingCount = result.candidateDetails.filter((c) => c.matched).length;
  ...
  const prevHostValuesRef = (0, react_1.useRef)(undefined);
  react_1.default.useEffect(() => {
  ...
  const pendingResourcesList = (0, react_1.useMemo)(() => {
    const pending = [];
  ...
  const handleViewModeChange = (0, react_1.useCallback)(
    (mode) => {
  ...
  return react_1.default.createElement(
            ...
            allowResourceCreationInternal &&
              react_1.default.createElement(
                'button',
                {
                  ...
                  className:
                    'flex items-center space-x-1 px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-500',
                  title: 'Create a new resource'
                },
                react_1.default.createElement(outline_1.PlusIcon, { className: 'h-4 w-4' }),
                react_1.default.createElement('span', null, 'Add Resource')
              )
          ),
          react_1.default.createElement(
            'div',
        ...
        react_1.default.createElement(
            ...
            react_1.default.createElement(
              ...
              lockedViewMode &&
                react_1.default.createElement(
                  ...
                  {
                    className:
                      'ml-2 inline-flex items-center px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full'
                  },
                  lockedViewMode.charAt(0).toUpperCase() + lockedViewMode.slice(1),
                  ' View'
                )
            ),
            resolutionState?.selectedResourceId &&
          ...
          react_1.default.createElement(
            ...
            !resolutionState?.selectedResourceId
              ...
              : react_1.default.createElement(ResolutionResults_1.ResolutionResults, {
    ...
    resolutionState?.newResourceDraft &&
      react_1.default.createElement(NewResourceModal_1.NewResourceModal, {
