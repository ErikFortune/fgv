...
const treeNavigation_1 = require('../../components/pickers/ResourcePicker/utils/treeNavigation');
describe('ResourcePicker Utilities', () => {
  describe('filterTreeBranch', () => {
    ...
    test('returns all resources when no rootPath provided', () => {
      const result = (0, treeNavigation_1.filterTreeBranch)(sampleResourceIds);
      expect(result).toEqual(sampleResourceIds);
    });
    test('filters to specific branch', () => {
      const result = (0, treeNavigation_1.filterTreeBranch)(sampleResourceIds, 'strings');
    ...
    test('filters with hideRootNode = true', () => {
      const result = (0, treeNavigation_1.filterTreeBranch)(sampleResourceIds, 'strings', true);
      expect(result).not.toContain('strings');
      expect(result).toContain('strings.common');
      expect(result).toContain('strings.common.ok');
    });
    test('handles non-existent rootPath', () => {
      const result = (0, treeNavigation_1.filterTreeBranch)(sampleResourceIds, 'nonexistent');
  ...
  describe('mergeWithPendingResources', () => {
    ...
    test('returns original list when no pending resources', () => {
      const result = (0, treeNavigation_1.mergeWithPendingResources)(existingIds);
      expect(result).toEqual(existingIds);
    });
    test('adds new pending resources', () => {
      const pending = [{ id: 'strings.new-resource', type: 'new', displayName: 'New Resource' }];
      const result = (0, treeNavigation_1.mergeWithPendingResources)(existingIds, pending);
      expect(result).toContain('strings.new-resource');
      expect(result).toContain('strings.common.ok');
    });
    test('removes deleted resources', () => {
      const pending = [{ id: 'strings.common.ok', type: 'deleted' }];
      const result = (0, treeNavigation_1.mergeWithPendingResources)(existingIds, pending);
    ...
    test('handles modified resources', () => {
      const pending = [{ id: 'strings.common.modified', type: 'modified', displayName: 'Modified Resource' }];
      const result = (0, treeNavigation_1.mergeWithPendingResources)(existingIds, pending);
  ...
  describe('adjustResourcePath', () => {
    test('returns original path when no rootPath provided', () => {
      const result = (0, treeNavigation_1.adjustResourcePath)('strings.common.ok');
      expect(result).toBe('strings.common.ok');
    });
    test('returns original path when hideRootNode is false', () => {
      const result = (0, treeNavigation_1.adjustResourcePath)('strings.common.ok', 'strings', false);
      expect(result).toBe('strings.common.ok');
    });
    test('adjusts path when hideRootNode is true', () => {
      const result = (0, treeNavigation_1.adjustResourcePath)('strings.common.ok', 'strings', true);
      expect(result).toBe('common.ok');
    });
    test('returns original path when not under rootPath', () => {
      const result = (0, treeNavigation_1.adjustResourcePath)('app.ui.buttons', 'strings', true);
      expect(result).toBe('app.ui.buttons');
    });
  });
  describe('getResourceDisplayName', () => {
    test('returns full path when using dot notation', () => {
      const result = (0, treeNavigation_1.getResourceDisplayName)('strings.common.ok');
      expect(result).toBe('strings.common.ok');
    });
    test('handles adjusted paths with hideRootNode', () => {
      const result = (0, treeNavigation_1.getResourceDisplayName)('strings.common.ok', 'strings', true);
      expect(result).toBe('common.ok');
    });
    test('handles single segment paths', () => {
      const result = (0, treeNavigation_1.getResourceDisplayName)('strings');
  ...
  describe('buildResourceTree', () => {
    test('throws error on duplicate conflicts', () => {
      ...
      expect(() => (0, treeNavigation_1.buildResourceTree)(conflictingIds)).toThrow(
        'test: Duplicate resource at path.'
      );
    });
    test('handles empty input', () => {
      const result = (0, treeNavigation_1.buildResourceTree)([]);
      expect(result).toEqual([]);
    });
  ...
  describe('flattenTree', () => {
    test('handles empty tree', () => {
      const result = (0, treeNavigation_1.flattenTree)([]);
      expect(result).toEqual([]);
    });
  ...
  describe('searchResources', () => {
    ...
    test('returns all resources when no search term', () => {
      const result = (0, treeNavigation_1.searchResources)(sampleIds, '');
      expect(result).toEqual(sampleIds);
    });
    test('filters by search term', () => {
      const result = (0, treeNavigation_1.searchResources)(sampleIds, 'common');
      expect(result).toContain('strings.common.ok');
      expect(result).toContain('strings.common.cancel');
    ...
    test('searches within current branch only', () => {
      const result = (0, treeNavigation_1.searchResources)(sampleIds, 'config', 'current-branch', 'app');
      expect(result).toContain('app.config.settings');
      expect(result).not.toContain('strings.common.ok');
    });
    test('searches all branches when scope is all', () => {
      const result = (0, treeNavigation_1.searchResources)(sampleIds, 'settings', 'all');
      expect(result).toContain('app.config.settings');
    });
  });
});
...
describe('ResourcePicker Integration', () => {
  test('branch isolation works with pending resources', () => {
    ...
    const allIds = (0, treeNavigation_1.mergeWithPendingResources)(existingIds, pending);
    // Apply branch isolation
    const stringsOnly = (0, treeNavigation_1.filterTreeBranch)(allIds, 'strings');
  ...
  test('complex branch isolation with multiple pending changes', () => {
    ...
    const allIds = (0, treeNavigation_1.mergeWithPendingResources)(existingIds, pending);
    const stringsOnly = (0, treeNavigation_1.filterTreeBranch)(allIds, 'strings', true);
