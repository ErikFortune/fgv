<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [@fgv/ts-web-extras](./ts-web-extras.md) &gt; [FileApiTreeAccessors](./ts-web-extras.fileapitreeaccessors.md) &gt; [createFromFiles](./ts-web-extras.fileapitreeaccessors.createfromfiles.md)

## FileApiTreeAccessors.createFromFiles() method

> Warning: This API is now obsolete.
> 
> Use create() with TreeInitializer array instead
> 

Legacy method: Create FileTree from File API files by pre-loading all content.

**Signature:**

```typescript
static createFromFiles(files: IFileApiFile[], prefix?: string): Promise<Result<FileTree.FileTree>>;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

files


</td><td>

[IFileApiFile](./ts-web-extras.ifileapifile.md)<!-- -->\[\]


</td><td>

Array of files with their paths


</td></tr>
<tr><td>

prefix


</td><td>

string


</td><td>

_(Optional)_ Optional prefix to add to all paths


</td></tr>
</tbody></table>

**Returns:**

Promise&lt;Result&lt;FileTree.FileTree&gt;&gt;

Promise resolving to a FileTree with all content pre-loaded

