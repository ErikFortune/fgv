...
import type { IProcedureState, IProcedureActions } from './model';
...
export interface IProcedureSelectorProps {
  ...
  /**
   * Callback when user wants to add a new procedure option.
   * If not provided, the "Add procedure option" button is hidden.
   * The callback should open a picker and call actions.addOption() with the selected option.
   */
  onAddProcedure?: () => void;
  /**
 ...
 *
 * Provides selection from available procedures, with optional ability to
 * add new procedures and remove existing ones from the options.
 *
 * @example
 * ```tsx
 ...
 * @returns Procedure selector element
 * @public
 */
export function ProcedureSelector({
  ...
  label = 'Procedure',
  disabled = false
}: IProcedureSelectorProps): React.ReactElement | null {
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {canRemove && state.effectivePreferredId && (
            <button
              ...
              title="Remove this procedure from options"
            >
              Remove
            </button>
          )}
      ...
      {!hasOptions && (
        <div className="text-xs text-gray-500 dark:text-gray-400 italic">No procedures configured</div>
      )}
