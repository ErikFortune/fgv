...
export function ServicesSection(props: IServicesSectionProps): React.ReactElement {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-primary mb-1">Services</h2>
      ...
      {/* Proxy URL + status */}
      <div>
        <label htmlFor="services-proxy-url" className="block text-xs font-medium text-muted mb-1">
          Backend URL
        </label>
        <div className="flex items-center gap-3">
          <input
            ...
            type="text"
            className="w-full max-w-[400px] px-2 py-1 text-xs border border-border rounded focus:outline-none focus:ring-1 focus:ring-focus-ring focus:border-transparent"
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <span className="text-xs text-muted">{statusLabel(healthStatus)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3">
        <input
          ...
          className="mt-0.5 w-4 h-4 rounded border-border text-brand-accent focus:ring-focus-ring disabled:opacity-30"
        />
        <label
          htmlFor="services-proxy-all"
          className={`text-sm ${proxyUrl ? 'text-secondary' : 'text-muted'}`}
        >
