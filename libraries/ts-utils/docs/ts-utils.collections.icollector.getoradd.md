<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [@fgv/ts-utils](./ts-utils.md) &gt; [Collections](./ts-utils.collections.md) &gt; [ICollector](./ts-utils.collections.icollector.md) &gt; [getOrAdd](./ts-utils.collections.icollector.getoradd.md)

## Collections.ICollector.getOrAdd() method

Gets an item by key if it exists, or creates a new item and adds it using the default [factory](./ts-utils.collections.collectiblefactory.md) if not.

**Signature:**

```typescript
getOrAdd(key: TKEY, item: TSRC): DetailedResult<TITEM, ResultMapResultDetail>;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

key


</td><td>

TKEY


</td><td>

The key of the item to retrieve.


</td></tr>
<tr><td>

item


</td><td>

TSRC


</td><td>

The source representation of the item to be added if it does not exist.


</td></tr>
</tbody></table>
**Returns:**

[DetailedResult](./ts-utils.detailedresult.md)<!-- -->&lt;TITEM, [ResultMapResultDetail](./ts-utils.collections.resultmapresultdetail.md)<!-- -->&gt;

Returns [Success](./ts-utils.success.md) with the item if it exists or could be created, or [Failure](./ts-utils.failure.md) with an error if the item cannot be created and indexed.

