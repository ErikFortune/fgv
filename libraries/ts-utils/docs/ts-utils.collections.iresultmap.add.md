<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [@fgv/ts-utils](./ts-utils.md) &gt; [Collections](./ts-utils.collections.md) &gt; [IResultMap](./ts-utils.collections.iresultmap.md) &gt; [add](./ts-utils.collections.iresultmap.add.md)

## Collections.IResultMap.add() method

Sets a key/value pair in the map if the key does not already exist.

**Signature:**

```typescript
add(key: TK, value: TV): DetailedResult<TV, ResultMapResultDetail>;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

key


</td><td>

TK


</td><td>


</td></tr>
<tr><td>

value


</td><td>

TV


</td><td>


</td></tr>
</tbody></table>

**Returns:**

[DetailedResult](./ts-utils.detailedresult.md)<!-- -->&lt;TV, [ResultMapResultDetail](./ts-utils.collections.resultmapresultdetail.md)<!-- -->&gt;

